...
## Reading layer `PROVCenso2010' from data source `/home/ingan/unidad-1-asignacion-1-datos-espaciales-en-r-andresmoreta/data/divisionRD.gpkg' using driver `GPKG'
...
``` r
plot(prov[prov$PROV %in% '07', 'PROV'])
```
...
## geometry type:  MULTIPOLYGON
## dimension:      XY
## bbox:           xmin: 196067.7 ymin: 2064387 xmax: 256252.6 ymax: 2134244
## epsg (SRID):    32619
## proj4string:    +proj=utm +zone=19 +datum=WGS84 +units=m +no_defs
...
## proj4string:    +proj=utm +zone=19 +datum=WGS84 +units=m +no_defs
##   PROV REG  TOPONIMIA ENLACE                           geom
## 1   07  07 ELÍAS PIÑA   0707 MULTIPOLYGON (((235630.8 21...
##           areaenm2
## 1 1395600062 [m^2]
prov %>% dplyr::filter(PROV %in% '07') %>% dplyr::select(areaenm2) %>% plot
```
...
``` r
prov %>% dplyr::filter(PROV %in% c('07', '10', '05')) %>% dplyr::select(areaenm2) %>% plot
```
...
## Reading layer `MUNCenso2010' from data source `/home/ingan/unidad-1-asignacion-1-datos-espaciales-en-r-andresmoreta/data/divisionRD.gpkg' using driver `GPKG'
...
``` r
mimun <- mun %>% dplyr::filter(PROV %in% '07')
mimun %>% plot
```
